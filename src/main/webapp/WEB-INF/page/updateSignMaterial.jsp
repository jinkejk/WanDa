...
<script type="text/javascript">
    ...
    //上传
    function uploadFile() {
	   if(validate() && confirm("上传会覆盖原先图纸！确定上传？")){
...
<br>
	<center>
	   更换文件：<form action="uploadUtilAction_uploadSignMaterial" id="upload_file" method="post" enctype="multipart/form-data">
	       <input id="fileUpload" name="fileUpload" type="file"/>
		   <input type="button" value="上传" onclick="uploadFile()">
		   <input type="hidden" name="SMId" id="SMId" value="${signMaterial.SMId}">
           <input type="hidden" name="originFileName" value="${signMaterial.signFile}">  
           <div id="upload_result"></div>
        </form><br>        
		<form action="operateSignMaterial_uploadSM" onsubmit="return check()" method="post">
			<ul>
			    <li>
				标题：<input type="text" id="title" name="title" value="${signMaterial.title}"
					style="width: 650px; height: 30px"><br><br>
			    </li>
			    <li>
				备注：<input type="text" id="remark" name="remark" value="${signMaterial.remark}"
					style="width: 650px; height: 30px"><br><br>
			    </li>
				<li>	
		          密级： 
			<select name="securityLevelName">
			    <c:forEach var="securityLevel" items="${securityLevels}">
			        <c:if test="${signMaterial.securityLevel.securityLevelName==securityLevel.securityLevelName}">
			...
			<select id="firstLevelTMCId" name="firstLevelTMCId" onchange="getSecondLevelTMCs()">
			    <c:forEach var="firstLevelTMC" items="${firstLevelTMCs}">
			        <c:if test="${signMaterial.category.TMCId==firstLevelTMC.TMCId}">
			...
			<c:if test="${!empty secondLevelTMCs}">
			    <c:forEach var="secondLevelTMC" items="${secondLevelTMCs}">
			        <c:if test="${signMaterial.category.TMCId==secondLevelTMC.TMCId}">
			...
			<div id="msg"></div>
			<input type="hidden" name="SMId" id="SMId" value="${signMaterial.SMId}">	
			<input type="hidden" id="signFile" name="signFile" value="${signMaterial.signFile}">
