			...
			<div class="z_list2">
				<div class="zx_list21">新增资料通知
					<form action="searchSolution" method="post" id="form1" onsubmit="return search()">
						...
						<input type="text" class="z_topn2" id="searchContent" name="searchContent" value="${searchContent}" onfocus="if(value=='请输入你要搜索的内容') {value=''}" onblur="if (value=='') {value='请输入你要搜索的内容'}">
					</form>
				</div>
						...
						<c:if test="${solutions.size() > 0}">
							<c:forEach var="solution" items="${solutions}" varStatus="status">
								<li><div class="zx_l1">
									...
									<div class="x_r1"><span class="zx_r2">New</span><span class="zx_r22">${fn:substring(solution.createDate,0,10)}</span></div>
								</li>
							</c:forEach>
						</c:if>
						<c:if test="${empty solutions || solutions.size() <= 0}">
							<li><div class="zx_l1"> 无相关内容.......</div></li>
						</c:if>
					</ul>
				</div>
			</div>
