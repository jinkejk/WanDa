...
(function ($) {
    $.fn.hovertree = function (options) {
        var settings = $.extend({width: "", isCloseOther: false, initStatus: "keleyi"}, options);
        ...
        liSet.on("mouseenter", function () {
            $(this).css({"overflow": "visible", "background-color": "#eee"});
            var pleft = $(this).position().left, ptop = $(this).position().top;
            var pwidth = $(this).width(), ppaddingLeft = parseInt($(this).css("padding-left"));
            $(this).find(">ul").css({
                "position": "absolute",
                "left": (pleft + pwidth + ppaddingLeft).toString() + "px",
                "top": ptop.toString() + "px",
                "border": "solid 1px #ccc",
                "z-index": "2014"
            }).show()
        });
        liSet.on("mouseleave", function () {
            $(this).css({"overflow": "hidden", "background-color": "transparent"});
            $(this).find(">ul").hide()
        })
    }
}(jQuery));